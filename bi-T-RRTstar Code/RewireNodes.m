...
function [pathTree,Tree] = RewireNodes(pathTree,Tree,extendedNode,nearNodes,...
    PREM,figHandle,mapSize,dX,dY,PURM,X_Grid,Y_Grid,Population,Constraints,T)
...
if length(nearNodes) > 1
    ...
    for jj = 1:length(nearNodes)
        ...
        if (nearNodes{jj}.ID ~= pathTree(extendedNode.ID).parent)&&((abs(angDiff)/delta_t_max)<=psiDot_Constraint)
            [riskExposureAlongNearNode,riskExtendNode,~,~] = RiskExposureSegment(pathTree,PREM,[extendedNode.x extendedNode.y],nearNodes{jj}.ID,mapSize(:,1),mapSize(:,2),dX,dY,PURM,X_Grid,Y_Grid,Population,T);
...
%                 end
    ...
    for jj = 1:length(nearNodes)
        ...
        if (nearNodes{jj}.ID ~= pathTree(extendedNode.ID).parent)&&((abs(angDiff)/delta_t_max)<=psiDot_Constraint)
            ...
            [riskExposureAlongExtendedNode,riskNearNode,~,~] = RiskExposureSegment(pathTree,PREM,[nearNodes{jj}.x nearNodes{jj}.y],extendedNode.ID,mapSize(:,1),mapSize(:,2),dX,dY,PURM,X_Grid,Y_Grid,Population,T);
