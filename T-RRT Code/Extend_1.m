...
%% EXTEND FUNCTION
function [pathTree,Tree,extended,T] = Extend_1(nearest, target, stepSize,nearestNodeID,pathTree,Tree,PREM,mapSize,...
    ...
    if ((abs(angDiff)/delta_t_max)<=psiDot_Constraint)&&(dist <= stepSize)
        ...
        [riskExposureAlongSegment_1,PositionRisk_1,collision_1,T] = collCheck(target,nearestNodeID,pathTree,PREM,mapSize,...
            dX,dY,PURM,X_Grid,Y_Grid,Population,RiskThreshold,TotalLength_1,obstacles,T);
    ...
    elseif ((abs(angDiff)/delta_t_max)>psiDot_Constraint)&&(dist <= stepSize)
        ...
        [riskExposureAlongSegment_1,PositionRisk_1,collision_1,T] = collCheck(newPos1,nearestNodeID,pathTree,PREM,mapSize,...
            dX,dY,PURM,X_Grid,Y_Grid,Population,RiskThreshold,TotalLength_1,obstacles,T);
