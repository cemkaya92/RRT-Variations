...
tic
    ...
    T.rate = 0.3; % transition rate [0 - 1]
    T.range = 10^(-11); % initial cost range between max and min RiskDensity
    T.temp = 10^(-6); % Transition Temperature
    
    %% Kinematic/Dynamic Constraints
    Constraints.psi_dot = (160)*pi/180;
...
%         obstacles = polygonalObstacles(mapSize,figHandle);
    ...
    [MultiplePaths,finalPath,FinalSmoothedPath,riskExposureLevels]...
        ...
        V_des,PREM,dX,dY,PURM,X,Y,Population,RiskThreshold,obstacles,ResMapSize,Constraints,...
        plotBranches,RecordVideo,randNodeExtProb,p_quasi,pathFigs,T);
