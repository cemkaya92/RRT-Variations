...
function [riskExposureAlongPath,PositionRisk] = RiskExposureSegment(pathTree,PREM,extended,...
    PathNodeID,xrange,yrange,dX,dY,PURM,X,Y,Population)


    x1 = extended(1);  y1 = extended(2); extendedGrid = [ceil((x1-xrange(1))/dX);ceil((y1-yrange(1))/dY) ];
    x2 = pathTree(PathNodeID).x;  y2 = pathTree(PathNodeID).y; pathNodeGrid = [ceil((x2-xrange(1))/dX);ceil((y2-yrange(1))/dY) ];
