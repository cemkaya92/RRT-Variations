...
function [riskExposureAlongPath,PositionRisk,collision] = collCheck(newPosition,nearestNodeID,pathTree,PREM,mapSize,...
...
if ((TotalLength>LengthThreshold)||PolygonCheck||(newPosition(1)<mapSize(1,1))||(newPosition(1)>mapSize(2,1))...
    ...
    [riskExposureAlongPath,PositionRisk] = RiskExposureSegment(pathTree,PREM,newPosition,...
        nearestNodeID,mapSize(:,1),mapSize(:,2),dX,dY,PURM,X_Grid,Y_Grid,Population);
    
%     if ((PositionRisk.cost>ExposureRateThreshold) || (riskExposureAlongPath>CumulativeRiskThreshold))
    if ((PositionRisk.cost>ExposureRateThreshold))
        collision = true;
    else
        collision = false;
    end
